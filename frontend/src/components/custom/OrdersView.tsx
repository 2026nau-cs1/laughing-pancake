...
import { Package, CheckCircle, Truck, Clock, XCircle, Star } from 'lucide-react';
  ...
  return (
    <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-24 md:pb-8">
      ...
      <div className="flex gap-1 p-1 rounded-xl mb-6" style={{ backgroundColor: '#F7F3EE', border: '1px solid #E2D9CF' }}>
        {[
          { id: 'buying' as const, label: `我购买的 (${buyingOrders.length})` },
          { id: 'selling' as const, label: `我出售的 (${sellingOrders.length})` },
        ].map((tab) => (
          <button
            key={tab.id}
      ...
      {loading ? (
        <div className="space-y-3">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="h-28 rounded-xl animate-pulse" style={{ backgroundColor: '#E2D9CF' }}></div>
          ))}
        </div>
        ...
        <div className="space-y-4">
          {displayOrders.map((order, idx) => {
            ...
            return (
              <div key={order.id} className="rounded-xl overflow-hidden" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E2D9CF' }}>
                {/* Order Header */}
                <div className="flex items-center justify-between px-4 py-3" style={{ backgroundColor: '#F7F3EE', borderBottom: '1px solid #E2D9CF' }}>
                  ...
                  <span className="text-xs" style={{ color: '#6B6560' }}>{new Date(order.createdAt).toLocaleDateString('zh-CN')}</span>
                </div>
                ...
                <div className="p-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-sm" style={{ color: '#1A1A1A' }}>{order.bookTitle}</h3>
                      <p className="text-xs mt-0.5" style={{ color: '#6B6560' }}>
                        {isBuyer ? `卖家: ${order.sellerName}` : `买家: ${order.buyerName}`}
                      </p>
                      {order.shippingAddress && (
                        <p className="text-xs mt-0.5 truncate" style={{ color: '#6B6560' }}>地址: {order.shippingAddress}</p>
                      )}
                      {order.trackingNumber && (
                        <p className="text-xs mt-0.5" style={{ color: '#2D4A3E' }}>快递单号: {order.trackingNumber}</p>
                      )}
                    </div>
                    <div className="text-right flex-shrink-0">
                      <div className="text-lg font-bold" style={{ color: '#C8873A' }}>￦{Number(order.price).toFixed(2)}</div>
                      <div className="text-xs mt-0.5" style={{ color: '#6B6560' }}>{order.paymentMethod || '未选择'}</div>
                    </div>
                  </div>
      ...
      {showReviewModal && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <div className="w-full max-w-md rounded-2xl p-6" style={{ backgroundColor: '#FFFFFF' }}>
            <h3 className="text-lg font-bold mb-4" style={{ fontFamily: 'Georgia, serif', color: '#1A1A1A' }}>交易评价</h3>
            <div className="mb-4">
              <label className="text-xs font-medium mb-2 block" style={{ color: '#6B6560' }}>评分</label>
            ...
            <div className="mb-5">
              <label className="text-xs font-medium mb-1.5 block" style={{ color: '#6B6560' }}>评价内容</label>
              <textarea
                value={reviewComment}
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowReviewModal(null)} className="flex-1 py-3 rounded-xl text-sm font-medium" style={{ backgroundColor: '#F7F3EE', color: '#6B6560' }}>取消</button>
              <button onClick={handleSubmitReview} disabled={submitting} className="flex-1 py-3 rounded-xl text-sm font-semibold text-white disabled:opacity-60" style={{ backgroundColor: '#2D4A3E' }}>
                {submitting ? '提交中...' : '提交评价'}
              </button>
            </div>
          </div>
